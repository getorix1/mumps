...
ExampleCalendar = GTK_CALENDAR(gtk_builder_get_object(builder, "ExampleCalendar"));
	{
	char tmp[128];
 	sprintf(tmp,"%p",ExampleCalendar);
 	SymPut("ExampleCalendar", tmp);
 	fprintf(f, " set ExampleCalendar=\"%s\"\n", tmp);
 	}

SampleGtkScale = GTK_SCALE(gtk_builder_get_object(builder, "SampleGtkScale"));
	{
	char tmp[128];
 	sprintf(tmp,"%p",SampleGtkScale);
 	SymPut("SampleGtkScale", tmp);
 	fprintf(f, " set SampleGtkScale=\"%s\"\n", tmp);
 	}

SampleGtkScrollBar = GTK_SCROLLBAR(gtk_builder_get_object(builder, "SampleGtkScrollBar"));
	{
	char tmp[128];
 	sprintf(tmp,"%p",SampleGtkScrollBar);
 	SymPut("SampleGtkScrollBar", tmp);
 	fprintf(f, " set SampleGtkScrollBar=\"%s\"\n", tmp);
 	}

SampleGtkSpinButton = GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "SampleGtkSpinButton"));
	{
	char tmp[128];
 	sprintf(tmp,"%p",SampleGtkSpinButton);
 	SymPut("SampleGtkSpinButton", tmp);
 	fprintf(f, " set SampleGtkSpinButton=\"%s\"\n", tmp);
 	}
